 ...
 * objects.
 * We are using a clockwise point ordering convention(user for normal calculating)
 */
class Polygon {
  /*
  ...
  normals = new PVector[pointsTransformed.length];
    for (int i=0; i<pointsTransformed.length; i++) {
      ...
      normals[i] = new PVector(-edge.y, edge.x).normalize();
    }
  ...
  void draw() {
    ...
    // draw red if colliding this frame
    // draw the shape:
    beginShape();
    for (int i = 0; i < pointsTransformed.length; i++) {
